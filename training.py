...
def kld_gmm(mu, log_var, strength, mode=None):
    ...
    if mode is None:
        ...
        all_strengths = all_strengths.to(device)
...
def train_emd_vae(net, train_loader, test_loader, epochs, optimizer, recon_weight=1., kl_weight=1., reg_weight=0,
    ...
    for epoch in range(epochs):
        ...
        test_loss, test_recon_loss, test_kld_loss = test_emd_vae(net=net, test_loader=test_loader,
...
def test_emd_vae(net, test_loader, recon_weight, kl_weight, nn_type, gmm=1, reg_weight=0, power=0, kl_mode=None):
    net.eval()
    net = net.to(device)
    test_loss = 0.
    recon_loss = 0.
