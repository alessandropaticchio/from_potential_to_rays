...
strengths = [0.01, 0.3]
dataset = 'potential'
model_name = 'potential_VAE_[0.01, 0.3]_2021-07-26 07_40_15.580612.pt'
...
if dataset == 'potential':
    pics_train_dataset = torch.load(DATA_ROOT + 'RP_images/loaded_data/' + 'training_' + dataset + '_unscaled.pt')
    pics_test_dataset = torch.load(DATA_ROOT + 'RP_images/loaded_data/' + 'test_' + dataset + '_unscaled.pt')
else:
    pics_train_dataset = torch.load(DATA_ROOT + 'RP_images/loaded_data/' + 'training_' + dataset + '_unscaled.pt')
    pics_test_dataset = torch.load(DATA_ROOT + 'RP_images/loaded_data/' + 'test_' + dataset + '_unscaled.pt')
...
if transform:
    ...
    plt.subplot(1, 2, 1)
    plt.title('Original')
    plt.imshow(rand_sample.squeeze(), cmap='gray', vmin=0, vmax=float(rand_strength.item()))

    plt.subplot(1, 2, 2)
    plt.title('Reconstruction')
    plt.imshow(rand_sample_prime.squeeze().detach().numpy(), cmap='gray', vmin=0, vmax=float(rand_strength.item()))
